...
class Debug
{
	...
	type(v)
	{
		...
		return $out([r],this.name,$red,this.bg,this.dt());
	}
...
},{"_process":13,"buffer":11}],2:[function(require,module,exports){
/**
 * AM
 */
"use strict";
...
const ATerm = require('./io/ATerm');

class AM
{
	
	constructor(fps=32,opt)
	{
		this.fps = fps;
		this.term = [];
		this.opt = opt;
		...
		this.create(opt); 
		if (fps > 0) setInterval(this.update,1000/fps,this);
		else this.update(this);
	}	
	
	create(opt)
	{
		this.addTerm(new ATerm(opt));
	}
	...
	addLayer(l)
	{
		for (let t of this.term) t.addLayer(l)
	}
	
	getLayer(id)
	{
		let l = null;
		for (l of this.term[0].layer) if (l.name == id) break;
		return l;
	}
...
},{"./io/ATerm":4,"abv-ts":1}],3:[function(require,module,exports){
/**
 * Node
 */
"use strict";
...
class Node{
	constructor(id,x=0,y=0,w=0,h=0,c="blue")
	{
		this.id = id;
		this._x = x;
		this._y = y;
...
},{"abv-ts":1}],4:[function(require,module,exports){
/**
 * ATerm
 */
"use strict";
...
//		this.doc = opt.doc || null; 
	}	
	
	render()
	{
		this.clear();
		for (let l of this.layer) this.draw(l);
	}
	...
	{
		switch(el.kind){
			case 0: this.rect(el.id,el.x,el.y,el.w,el.h,el.color); break;
...
},{"abv-ts":1}],5:[function(require,module,exports){
/**
 * Term0D
 */
"use strict";
...
class Term0D extends ATerm
{
	...
	rect(id,x,y,w,h,c)
	{
		...
		h = Math.round(h/m);
		if (h < 3) h = 3;
	//	ts.debug('rect; ',x,y,w,h,c);
		ts.println(ts.clr2c(c,true) + $pos(x,y) + $ul(w));
		for (let i=1;i<h-2;i++) ts.println($pos(x,y+i) + $ml(w));
		ts.println($pos(x,y+h-2) + $dl(w));
		ts.println(ts.clr2c());
//		s += $h;
...
},{"./ATerm":4,"abv-ts":1}],6:[function(require,module,exports){
/**
 * Term1D
 */
"use strict";
...
},{"./ATerm":4,"abv-ts":1}],7:[function(require,module,exports){
/**
 * Term2D
 */
"use strict";
...
//		ts.debug('rect: ',id,x,y,w,h);
		...
		this.ctx.fillStyle = c; //ts.debug(o.name,clr[o.c]);
		this.ctx.fill();
		this.ctx.stroke();
	}
...
},{"./Term1D":6,"abv-ts":1}],8:[function(require,module,exports){
/**
 * abv.io
 * @package abv.io
 */
...
const Term0D = require('./Term0D');
const Term1D = require('./Term1D');
const Term2D = require('./Term2D');
...
},{"./Term0D":5,"./Term1D":6,"./Term2D":7}],9:[function(require,module,exports){
/**
...
const ts = require('abv-ts')('abv:index');
const AM = require('./abv/AM');
const Node = require('./abv/Node');
const io = require('./abv/io/index');
...
},{"./abv/AM":2,"./abv/Node":3,"./abv/io/index":8,"abv-ts":1}],10:[function(require,module,exports){
'use strict'
