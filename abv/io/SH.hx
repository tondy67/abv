...
class SH{
	...
	public static function execute(script:String)
	{
		...
		try program = parser.parseString(script)
		catch(d:Dynamic){trace("Line: "+parser.line+": "+d);}
...
//
		ip.variables.set("TP",new TextProcessing());
...
//
		...
		try ip.execute(program) catch(d:Dynamic){trace("Line: "+parser.line+": "+d);}
